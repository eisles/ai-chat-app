...
export default function ProductImagesGeminiPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/gemini-product-images", {
        method: "POST",
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6 px-4 py-10 sm:px-6 lg:px-8">
      <div className="space-y-2">
        ...
        <h1 className="text-2xl font-semibold sm:text-3xl">
          Geminiでベクトル化して保存 (1408次元, image-embedding-001)
        </h1>
        <p className="text-sm text-muted-foreground">
          Google Gemini の image-embedding-001 を使って 1408 次元でベクトル化し、
          Neon の product_images_gemini に upsert します。
        </p>
      </div>
      ...
      <Card className="border bg-card/60 p-4 shadow-sm sm:p-6">
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              1408次元でベクトル化し、product_images_gemini に upsert します。
            </p>
          </div>
        </form>
      </Card>

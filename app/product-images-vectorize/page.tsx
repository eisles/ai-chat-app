...
export default function ProductImagesVectorizePage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/vectorize-product-images", {
        method: "POST",
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6 px-4 py-10 sm:px-6 lg:px-8">
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
          Image → Vector (Custom API)
        </p>
        <h1 className="text-2xl font-semibold sm:text-3xl">
          独自APIでベクトル化して保存 (512次元)
        </h1>
        <p className="text-sm text-muted-foreground">
          convertvectorapi.onrender.com に画像を送信し、Neon の
          product_images_vectorize に upsert します。
        </p>
      </div>
      ...
      <Card className="border bg-card/60 p-4 shadow-sm sm:p-6">
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              512次元でベクトル化し、product_images_vectorize に upsert します。
            </p>
          </div>
        </form>
      </Card>
        ...
        <Card className="border bg-card/60 p-4 shadow-sm sm:p-6">
          ...
          {result.ok ? (
            <div className="mt-3 space-y-1 text-sm">
              ...
              <div>cityCode: {result.cityCode}</div>
